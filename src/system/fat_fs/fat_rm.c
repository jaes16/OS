...
int fat_remove_dir_ent(fat_dir_entry *parent, fat_dir_entry *target, uint32_t target_loc, uint32_t parent_block)
{
	...
	// write block out again
	fat_write_blocks(buf, target_loc, 1);

	// update parent size in parent's '.'
	fat_read_blocks(buf, parent->cluster_num, 1);
	buf[0].file_size = parent->file_size;
	if(parent == fat_root_entry){
		// if parent is root, need to update '..' as well
		buf[1].file_size = parent->file_size;
		fat_write_blocks(buf, parent->cluster_num, 1);
	}
	else{
		fat_write_blocks(buf, parent->cluster_num, 1);
		//  if parent is not root, need to update parent size in grandparent directory blockas well
		fat_read_blocks(buf, parent_block, 1);
		for(uint32_t i = 0; i < fat_ents_per_blck(); i++){
			if (buf[i].cluster_num == parent->cluster_num){
				buf[i].file_size = parent->file_size;
				break;
			}
		}
...
int fat_rmdir (char *path)
{
	...
	Check if target directory permissions are writable
	Check if target directory is empty besides '.' and '..'
	Removal:
	Zero out all the disk blocks of target directory and free them up in the fat
	Remove the target directory entry from the parent directory and change parent directory size
	*/
  ...
  if (base_loc < 0) return base_loc;
	...
	// check if we have write permissions for parent directory
...
int fat_unlink(char *path)
{
	...
	Check if target file permissions are writable
	Removal:
	Zero out all the disk blocks of target directory and free them up in the fat
	Remove the target directory entry from the parent directory and change parent directory size
	*/
